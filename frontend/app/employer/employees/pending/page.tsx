    ...
    const fetchEmployees = useCallback(async () => {
        ...
        try {
            const response = await fetch(
                'http://localhost:8080/api/employer/employees?status=pending',
                {
                    headers: {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center flex-1 py-20">
                ...
                <p className="text-gray-500 mt-4">Loading pending employees...</p>
            </div>
        );
    }
            ...
            {filteredEmployees.length === 0 ? (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex-1 flex flex-col justify-center overflow-hidden min-h-[400px]">
                    <EmptyState
                        ...
                        description="Invite new employees to join your organization."
                    />
                </div>
            ) : (
                                ...
                                {filteredEmployees.map((emp, idx) => {
                                    ...
                                    return (
                                        <tr key={emp.id} className="hover:bg-gray-50 transition-colors group">
                                            ...
                                            {visibleColumns['team'] && <td className="p-4 font-medium text-brand-dark">—</td>}
                                            {visibleColumns['location'] && <td className="p-4 font-medium text-brand-dark">—</td>}
                                            {visibleColumns['status'] && (
                                                <td className="p-4">
                                                    <span className="bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-xs font-medium">Invitation Sent</span>
                                                </td>
                                            )}
