...
import { EmployeeActionsMenu } from '@/components/employer/EmployeeActionsMenu';
...
export default function ActiveEmployeesPage() {
    ...
    const COLUMN_DEFINITIONS = [
        ...
        { key: 'manualTime', label: 'Manual Time [h]', headerClass: "whitespace-nowrap" },
    ...
    const fetchEmployees = useCallback(async () => {
        ...
        try {
            const response = await fetch(
                'http://localhost:8080/api/employer/employees?status=active',
                {
                    headers: {
            ...
            {filteredEmployees.length === 0 ? (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex-1 flex flex-col justify-center overflow-hidden min-h-[400px]">
                    <EmptyState
                        ...
                        description="Employees who have completed onboarding will appear here."
                    />
                </div>
            ) : (
                        ...
                        <table className="border-collapse text-sm text-left w-full">
                            <thead className="bg-white sticky top-0 z-20">
                                <tr>
                                    ...
                                    <th className="p-4 font-medium text-gray-500 border-b border-gray-100 w-10"></th>
                                </tr>
                            </thead>
                                ...
                                {filteredEmployees.map((emp, idx) => {
                                    ...
                                    return (
                                        <tr key={emp.id} className="hover:bg-gray-50 transition-colors group">
                                            ...
                                            {visibleColumns['workTime'] && <td className="p-4 font-medium text-brand-dark">{emp.work_time}</td>}
                                            {visibleColumns['manualTime'] && <td className="p-4 font-medium text-brand-dark">{emp.manual_time}</td>}
                                            {visibleColumns['computerAct'] && <td className="p-4 font-medium text-brand-dark">{emp.computer_activity}</td>}
                                            {visibleColumns['productive'] && <td className="p-4 font-medium text-emerald-500">{emp.productive_time}</td>}
                                            {visibleColumns['unproductive'] && <td className="p-4 font-medium text-red-500">{emp.unproductive_time}</td>}
                                            <td className="p-4 text-right">
                                                <EmployeeActionsMenu
                                                    employeeId={emp.id}
                                                    employeeName={emp.fullname}
                                                    currentStatus="active"
                                                    onStatusChanged={fetchEmployees}
                                                />
                                            </td>
                                        </tr>
                                    );
