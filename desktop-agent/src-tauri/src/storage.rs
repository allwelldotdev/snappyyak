...
impl Storage {
    ...
    pub fn get_metrics_for_date(&self, date: &str) -> Result<Option<MetricsCache>> {
        let mut stmt = self.conn.prepare("SELECT date, work_time_minutes, computer_activity_minutes, productive_minutes, unproductive_minutes, neutral_minutes, last_sync FROM metrics_cache WHERE date = ?1")?;
        ...
        if let Some(row) = rows.next()? {
            Ok(Some(MetricsCache {
                ...
                productive_minutes: row.get(3)?,
                unproductive_minutes: row.get(4)?,
                neutral_minutes: row.get(5)?,
                last_sync: row.get(6)?,
            }))
        } else {
            Ok(None)
    ...
    pub fn update_metrics(&self, metrics: &MetricsCache) -> Result<()> {
        self.conn.execute(
            ...
            (date, work_time_minutes, computer_activity_minutes, productive_minutes, unproductive_minutes, neutral_minutes, last_sync) 
